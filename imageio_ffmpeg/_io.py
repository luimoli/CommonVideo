...
def count_frames_and_secs(path, seeking=None, decoder='copy'):
    """
    ...
    cmd = [_get_exe()] + seeking + ["-c:v", decoder, "-i", path,  "-map", "0:v:0", "-f", "null", "-"] # "-c", "copy" will produce an inaccurate result, however it's fast
...
def read_frames(
    ...
    * duration: duration in seconds. Can be zero if it could not be detected.
    ...
    logger.debug('Reader command line: ' + ' '.join(cmd))
...
def write_frames(
    ...
    # quality, bitrate, codec, macro_block_size can all be None or ...
    ...
    # Get command
    cmd = [_get_exe(), "-f", "rawvideo", "-vcodec", "rawvideo", "-s", sizestr]
    ...
    # Launch process
    logger.debug('Writer command line: ' + ' '.join(cmd))
